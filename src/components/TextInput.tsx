...
const TextInputInput = forwardRef<HTMLInputElement, TextInputInputProps>(({ children, color = 'cyan', error, ...props }: TextInputInputProps, ref) => {
  return (
    <div className={clsx("flex items-center gap-3 w-full  py-4 px-3 h-12 bg-gray-700 rounded hover:ring-1 focus-within:ring-1", {
      'hover:ring-cyan-500 focus-within:ring-cyan-500': color === 'cyan',
      'hover:ring-yellow-500 focus-within:ring-yellow-500': color === 'yellow',
      'ring-1 ring-red-500 focus-within:ring-red-500 hover:ring-red-500': error
    })}>
      {children}
      < input
        ref={ref}
